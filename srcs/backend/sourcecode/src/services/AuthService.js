...
class AuthService {
	...
	shouldAuthenticate(url) {
		const routes = ['/signin', '/signup', '/logout', '/google', '/callback', '/get-otp', 'validate-otp', 'refresh', 'pang'];
	...
	async canSignIn(body) {
		try {
			...
			if (user && (await bcrypt.compare(body.pass, user.pass)))
				// if user is null, it means the user does not exist
				return { stat: true };
	...
	async canSignUp(body) {
		try {
			...
			if (!serviceResult.user) return await this.userService.addUser(body);
			if (serviceResult.user.username === body.username) throw new Error('username already used');
	...
	async sendOTP(body) {
		try {
			...
			if (!user) throw new Error('no user exit with the provided email');
	...
	async refreshToken({ token }) {
		try {
			const decoded = jwt.verify(token, process.env.JWT_TOKEN_SECRET_REFRESH || 'salam kalam 3alam');

			const { user } = await this.userService.getUser(decoded.username);
			if (!user) return { stat: false, message: 'User not found' };

			const newAccessToken = jwt.sign({ userId: user.id, username: user.name }, process.env.JWT_TOKEN_SECRET || 'salam kalam 3alam', { expiresIn: '15m' });
