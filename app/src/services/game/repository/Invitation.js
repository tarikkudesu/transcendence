...
export function getInvitation(sender, recipient) {
    ...
    if (!invite)
        throw new Error(sender + recipient + ': no such invitation');
    return invite;
}
// * create invitation
export function createInvitation(sender, recipient, game) {
    ...
    if (rec.socket.PLAYFREE === false)
        throw new Error(rec.username + ' is currently playing');
    if (Main.repository.invitations.has(sen.username + rec.username))
        return;
...
export function deleteExpiredInvitations() {
    Main.repository.invitations.forEach((value, key) => {
        if (Date.now() - value.created_at > Main.invitationTimeout)
            Main.repository.invitations.delete(key);
    });
}
...
export function deleteAllRejectedInvitations(sender) {
    Main.repository.invitations.forEach((value, key) => {
        if (value.sender === sender && value.invite_status === 'declined')
            Main.repository.invitations.delete(key);
    });
}
...
export function getAllPlayerInvitations(username) {
    ...
    Main.repository.invitations.forEach((value) => {
        if (value.recipient === username)
            invitations.push(new Main.ClientInvitation(value.sender, value.game, value.invite_status));
    });
    return invitations;
}
