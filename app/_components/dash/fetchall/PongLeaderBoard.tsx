...
const PongLeaderBoard: React.FC<{ page: number }> = ({ page }) => {
	const router = useRouter();
	const fetchData = (): Promise<LeaderboardEntry[]> =>
		client.get(`/game/pong/leaderboard?begin=${page * 20}&end=${20}`).then((response) => response.data);
	const { data, error, isPending } = useQuery({
		queryKey: ['gamepongleaderboardall'],
		queryFn: fetchData,
	});
