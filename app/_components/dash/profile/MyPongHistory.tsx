...
import { fetchPongHistory } from '@/app/_service/game/calls';
...
const MyPongHistory: React.FC = ({}) => {
	const { username } = useAuth();
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const [isError, setError] = useState<boolean>(false);
	const [history, setHistory] = useState<PongHistoryEntry[]>([]);

	useEffect(() => {
		async function fetchData() {
			try {
				setIsLoading(true);
				const res: RequestResult = await fetchPongHistory(username);
		...
		return (
			<>
				{history.map((ele, index) => (
					<div key={index} className="grid grid-cols-6 grid-rows-5 gap-2">
